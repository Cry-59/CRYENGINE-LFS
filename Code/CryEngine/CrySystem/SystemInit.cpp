...
WIN_HMODULE CSystem::LoadDLL(const char* szModulePath, bool bQuitIfNotFound)
{
	...
	MEMSTAT_CONTEXT_FMT(EMemStatContextTypes::MSC_Other, 0, "%s", modulePath);
...
bool CSystem::UnloadEngineModule(const char* dllName)
{
	...
	stack_string modulePath = dllName;
	modulePath = CrySharedLibraryPrefix + PathUtil::ReplaceExtension(modulePath, CrySharedLibraryExtension);
...
L_done:;
		    ...
		    (m_sys_audio_disable->GetIVal() == 0))
		{
			...
			bAudioInitSuccess = InitializeEngineModule(DLL_AUDIOSYSTEM, cryiidof<IAudioSystemEngineModule>(), false);
		}
